...
class KubernetesCRDInjectionFunctor(cst.CSTTransformer):  # type: ignore[misc]
    """
    ...
    def _build_crd_attributes(self) -> list[cst.SimpleStatementLine]:
        return [
            self._build_urn_attribute(),
            cst.SimpleStatementLine(
                body=[
                    cst.Assign(
                        targets=[cst.AssignTarget(target=cst.Name(value="api_group"))],
                        value=cst.SimpleString(value=f'"{self.api_group}"'),
                    )
                ]
            ),
            cst.SimpleStatementLine(
                body=[
                    cst.Assign(
                        targets=[cst.AssignTarget(target=cst.Name(value="api_version"))],
                        value=cst.SimpleString(value=f'"{self.api_version}"'),
                    )
                ]
            ),
            cst.SimpleStatementLine(
                body=[
                    cst.Assign(
                        targets=[cst.AssignTarget(target=cst.Name(value="kind"))],
                        value=cst.SimpleString(value=f'"{self.kind}"'),
                    )
                ]
            ),
        ]
    ...
    def _build_class(self) -> cst.ClassDef:
        ...
        body_elements.extend(self._build_crd_attributes())
        ...
        return cst.ClassDef(
            name=cst.Name(value=self.crd_name),
            bases=[cst.Arg(value=cst.Name(value="KubernetesCRDBase"))],
    ...
    def _build_rebuild_call(self) -> cst.SimpleStatementLine:
        return cst.SimpleStatementLine(
            body=[
                cst.Expr(
                    value=cst.Call(
                        func=cst.Attribute(value=cst.Name(value=self.crd_name), attr=cst.Name(value="model_rebuild"))
                    )
                )
            ]
        )
    ...
    def leave_Module(self, original_node: cst.Module, updated_node: cst.Module) -> cst.Module:  # noqa: N802, ARG002
        ...
        for stmt in updated_node.body:
            if isinstance(stmt, cst.ClassDef) and stmt.name.value == self.crd_name:
                return updated_node
