...
def format_violations(response_data: dict) -> str:
    ...
    for v in response_data.get("violations", []):
        lines.append(f" - [{v.get('severity', 'CRITICAL')}] {v.get('rule_name')} in {v.get('file_name', 'unknown')}:{v.get('line_number', 0)} ({v.get('context_snippet', '')})")
    return "\n".join(lines)
...
async def execute_guillotine_scan(urn: str, payload_files: list) -> bool:
    ...
    server_path = os.environ.get(
        ...
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), "coreason-compliance-mcp", "src", "coreason_compliance_mcp", "scanner.py")
    )
    ...
    try:
            ...
            async with ClientSession(read, write) as session:
                ...
                try:
                    result = await asyncio.wait_for(
                        session.call_tool(
                            "evaluate_payload", 
                            arguments={"target_urn": urn, "files": payload_files}
                        ),
                        timeout=2.0 # 2000ms SLA
                    )
                except asyncio.TimeoutError:
                    raise KineticGuillotineError("ComplianceTimeout: Server took too long to evaluate. Publish aborted.")
        ...
        if isinstance(e, KineticGuillotineError):
            raise
        raise KineticGuillotineError(f"GuillotineConnectionError: Cannot verify payload safety. Details: {str(e)}")
